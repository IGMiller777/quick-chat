...
export class BroadcastService implements OnDestroy {
  ...
  private initializeUser(): void {
    ...
    let tabId = session?.getItem(TAB_ID_KEY) ?? undefined;
    let userId = session?.getItem(USER_ID_KEY) ?? undefined;
    let existingTabNumber = session?.getItem(TAB_NUMBER_KEY) ?? undefined;
    ...
    if (existingTabNumber) {
      ...
      try {
        session?.setItem(TAB_NUMBER_KEY, this.tabNumber.toString());
      } catch {}
    }
    ...
    if (!userId) {
      ...
      try {
        session?.setItem(USER_ID_KEY, userId);
      } catch {}
    }
  ...
  private getNextTabNumber(): number {
    try {
      const currentCounter = this._localStorage?.getItem(TAB_COUNTER_KEY) ?? null;
      const nextNumber = currentCounter ? parseInt(currentCounter, 10) + 1 : 1;

      this._localStorage?.setItem(TAB_COUNTER_KEY, nextNumber.toString());
  ...
  private cleanupTabNumber(): void {
    try {
      this._sessionStorage?.removeItem(TAB_NUMBER_KEY);
    } catch (error) {
